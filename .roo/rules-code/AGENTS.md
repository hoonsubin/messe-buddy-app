...
- **`verbatimModuleSyntax`** is on ([`tsconfig.app.json`](../../tsconfig.app.json:13)): use `import type` for type-only imports. Never mix runtime and type imports in one statement.
...
- **Components never call the adapter directly**. Business logic is in [`src/use-cases/`](../../src/use-cases/exportTemplate.ts). Components → use cases → adapter.
...
- **No `JSON.parse` on PB record fields** (C-13). [`FormSchemaRaw`](../../src/types/domain.ts:107) and [`ProgressEventRaw`](../../src/types/domain.ts:112) exist only inside the adapter. The adapter parses them into typed interfaces before returning to app-layer code.
...
- **Commented-out eslint-disable** in [`exportTemplate.ts`](../../src/use-cases/exportTemplate.ts:21) is intentional — destructuring with unused bindings for stripping PBRecord fields.
