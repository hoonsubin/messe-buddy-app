...
import type { Mission, Navigate, ProgressEvent } from "../types.ts";
import {
  getMilestoneById,
...
export default function PlayerCockpitPage(
  { sessionId, playerId, navigate }: PlayerCockpitPageProps,
) {
  const player = getPlayerById(playerId)!;
  const [progress, setProgress] = useState<ProgressEvent[]>(
    getProgressForPlayer(playerId),
  );
  ...
  const currentMilestoneId = MOCK_MILESTONES.find((m) =>
    m.status === "inProgress"
  )?.id;
  ...
  const pips = MOCK_MILESTONES.map((ms) => {
    ...
    const allDone = missions.length > 0 &&
      missions.every((m) => isMissionDone(m.id, progress));
    return { ms, allDone };
  });
    ...
    <div className="app-screen">
      ...
      <div className="ms-strip">
        <span style={{ fontSize: "0.72rem" }}>
          Day 23 of 90 ·{" "}
          {MOCK_MILESTONES.find((m) => m.id === currentMilestoneId)?.name ??
            "All done!"}
        </span>
          ...
          <span>
            {currentMilestoneId
              ? `${
                ...
                currentMissions.filter((m) => isMissionDone(m.id, progress))
                  .length
              }/${currentMissions.length} done`
              : ""}
          </span>
        </div>
        <div
          ...
          {currentMissions.map((mission) => {
            ...
            return (
              ...
              >
                ...
                {done ? <div className="mission-check">✓</div> : (
                  <div
