...
export default function AdminCockpitPage({ sessionId, navigate }: AdminCockpitPageProps) {
  const [selectedPlayerId, setSelectedPlayerId] = useState(MOCK_PLAYERS[0].id)
  const [selectedMilestoneId, setSelectedMilestoneId] = useState(MOCK_MILESTONES[4].id) // Hall A4
  const player = MOCK_PLAYERS.find(p => p.id === selectedPlayerId)!

  const selectedMilestone = MOCK_MILESTONES.find(m => m.id === selectedMilestoneId)!
  ...
  return (
    <div className="app-screen">
      <TopBar
        ...
        action={{ label: 'QR Scanner', onClick: () => navigate({ name: 'qrScanner', sessionId }), variant: 'green' }}
      />
      ...
      <div className="scroll">
        ...
        <div style={{ padding: '8px 12px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          {missionList.map(m => (
            <div key={m.id} className="mission-card" style={{ cursor: 'default' }}>
