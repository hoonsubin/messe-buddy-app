...
import { MOCK_MILESTONES, MOCK_PLAYERS } from "../mockData.ts";
...
export default function PlayerListPage(
  { sessionId, navigate }: PlayerListPageProps,
) {
      ...
      >
        <div className="row sb">
          <span className="text-sm text-muted">
            {MOCK_PLAYERS.length} players joined
          </span>
          <select
            className="input"
      ...
      <div className="scroll">
        ...
        >
          {MOCK_PLAYERS.map((player) => {
            ...
            const statusColor = pct >= 50
              ? "var(--success)"
              : pct >= 20
              ? "var(--brand)"
              : "var(--danger)";
            const statusLabel = pct >= 50
              ? "Ahead"
              : pct >= 20
              ? "On track"
              : "Needs attention";
            ...
            return (
              ...
              >
                ...
                <div>
                  <span
                    ...
                    style={{
                      ...
                      color: statusColor,
                      background: statusColor === "var(--danger)"
                        ? "var(--danger-bg)"
                ...
                <span style={{ color: "var(--text-subtle)", fontSize: "1rem" }}>
                  ›
                </span>
              </div>
            );
