...
function isPathActive(from: Milestone, to: Milestone) {
  return from.status === "completed" &&
    (to.status === "completed" || to.status === "inProgress");
}
    ...
    <div className="map-canvas">
      ...
      <svg className="map-svg">
        {MILESTONE_CONNECTIONS.map(([fromId, toId]) => {
          ...
          const strokeDash = to.status === "upcoming" || to.status === "locked"
            ? "4,3"
            : undefined;
          return (
            <line
...
function MilestoneNode(
  { milestone: ms, isActive, onClick }: MilestoneNodeProps,
) {
  const classNames = ["ms-node", ms.status, isActive ? "active" : ""].filter(
    Boolean,
  ).join(" ");
  const progressPct = ms.xpThreshold > 0
    ? (ms.earnedXP / ms.xpThreshold) * 100
    : 0;

  const fillColor = ms.status === "completed"
    ? "#2f9e44"
