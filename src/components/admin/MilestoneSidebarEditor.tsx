...
const MilestoneSidebarEditor = (props: MilestoneSidebarEditorProps) => {
  if (!props.milestone) {
    return (
      <div className="sidebar" data-testid="milestone-sidebar-editor">
        <p style={{ padding: "var(--space-6)", color: "hsl(var(--color-muted-fg))", fontSize: "var(--text-sm)" }}>
  ...
  return (
    <div className="sidebar sidebar--open" data-testid="milestone-sidebar-editor">
      <div style={{ padding: "var(--space-4) var(--space-5)", borderBottom: "1px solid hsl(var(--color-border))" }}>
        <h2 style={{ margin: 0, fontSize: "var(--text-lg)", fontWeight: "var(--weight-semibold)" }}>
          {props.milestone.name}
        </h2>
      </div>

      <div style={{ padding: "var(--space-3) var(--space-4)", borderBottom: "1px solid hsl(var(--color-border))" }}>
        <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
          {props.missions.map((m) => (
            <li key={m.id}>
              <button
                type="button"
                className={`btn btn--ghost${props.activeMissionId === m.id ? " btn--active" : ""}`}
                style={{ width: "100%", textAlign: "left", justifyContent: "flex-start", fontSize: "var(--text-sm)" }}
        ...
        <button type="button" className="btn btn--secondary" style={{ marginTop: "var(--space-3)", width: "100%" }} onClick={props.onAddMission}>
          + Add mission
        </button>
      </div>
      ...
      {props.draft && (
        <div style={{ padding: "var(--space-4) var(--space-5)", flex: 1, overflowY: "auto" }}>
          <MissionEditor draft={props.draft} onDraftChange={props.onDraftChange} />
        </div>
      )}

      <div style={{ padding: "var(--space-3) var(--space-4)", borderTop: "1px solid hsl(var(--color-border))" }}>
        <SaveActions
