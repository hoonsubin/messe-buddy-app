...
export default function MilestoneSidebarViewer({
  ...
  const doneMissions = missions.filter((m) =>
    isMissionDone(m.id, progressEvents)
  );
  const progressPct = Math.round(
    (milestone.earnedXP / milestone.xpThreshold) * 100,
  );
    ...
    <div className="overlay-backdrop" onClick={onClose}>
      <div className="overlay-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="panel-hdr">
          <div>
            ...
            <div className="pbar mt4" style={{ width: 200 }}>
              <div
                className={`pbar-fill ${
                  milestone.status === "completed" ? "success" : "primary"
                }`}
        ...
        >
          ...
          {tab === "FAQs" && (
            <div className="empty">
              FAQs for this milestone will appear here.
            </div>
          )}
          {tab === "People" && (
            <div className="empty">
              <div style={{ marginBottom: 12 }}>
                Key contacts for this milestone:
              </div>
              <div
                className="row"
              ...
              >
                {["Your Manager", "IT Support", "HR Team"].map((p) => (
                  <div key={p} style={{ textAlign: "center" }}>
                    <div className="avatar md" style={{ margin: "0 auto 4px" }}>
                      {p[0]}
                    </div>
                    <div
          ...
          {tab === "Resources" && (
            <div className="empty">
              Resources specific to {milestone.name} will appear here.
            </div>
          )}
        </div>
